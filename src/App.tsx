...
function UserGroups({ state, token }: { token: string; state: AdminState | undefined }) {
  ...
  const kickUser = useMutation(api.game.kickUser);
        ...
        {waiting.map((user) => (
          <div className="user-row" key={user._id}>
            ...
            <span className="user-actions">
              ...
              <KickUserButton
                userName={user.name}
                onKick={() => kickUser({ adminToken: token, userId: user._id as Id<"users"> })}
              />
            </span>
          </div>
        ))}
...
function KickUserButton({ userName, onKick }: { userName: string; onKick: () => Promise<unknown> }) {
  return (
    <button
